...
async def ask_question(
    ...
    # salviamo i valori della risposta nella nostra tabella nel db
...
async def edit_state(
    ...
    # aggiorniamo lo stato con aupdate_state() e con la answer modificata
    await human_workflow.workflow.aupdate_state(
        config={"configurable": {"thread_id": thread_id}},
        values={"answer": request.answer},
    )
    # aggiorniamo l'attributo answer nella nostra tabella
...
async def confirm(thread_id: str, db: Session = Depends(get_db)):
    ...
    # si riprende il flusso dopo l'intterupt
    response_state = await human_workflow.ainvoke(
        input=None,
        config={"configurable": {"thread_id": thread_id}},
    )
    # sovrascriviamo gli attribute confirmed e answer nella nostra tabella
...
async def delete_thread(thread_id: str, db: Session = Depends(get_db)):
    ...
    if not thread:
        raise HTTPException(status_code=404, detail="Thread ID does not exist.")
    # se il thread esiste allora lo eliminiamo dalla tabella
    # e non lo vedremo più nell'UI
    db.delete(thread)
    db.commit()
    return ThreadResponse(
        thread_id=thread.thread_id,
